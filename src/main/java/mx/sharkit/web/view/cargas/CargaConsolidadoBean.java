...
@Named
@ViewScoped
public class CargaConsolidadoBean implements Serializable, Observer {

    private static final Logger LOGGER = Logger.getLogger(CargaConsolidadoBean.class.getName());
    private static final String CLAVE_PROCESO = "ARCHIVOS_CARGA";
    private static final String CLAVE_SCRIPT = "CARGA_CONSOLIDADO";

    @Autowired
    ArchivoCargaService archivoCargaService;

    @Autowired
    ChipConsolidadoService chipConsolidadoService;
        ...
        try {
            ...
            if (errores.isEmpty()) {
                if (!chips.isEmpty()) {
                    ...
                    Integer actualizados = chipConsolidadoService.saveChipConsolidado(chips);
