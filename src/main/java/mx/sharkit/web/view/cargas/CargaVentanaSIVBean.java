...
@Named
@ViewScoped
public class CargaVentanaSIVBean implements Serializable, Observer {

    private static final Logger LOGGER = Logger.getLogger(CargaVentanaSIVBean.class.getName());
    private static final String CLAVE_PROCESO = "ARCHIVOS_CARGA";
    private static final String CLAVE_SCRIPT = "CARGA_VENTANA_SIV";

    @Autowired
    ArchivoCargaService archivoCargaService;

    @Autowired
    ChipVentanaSivService chipVentanaSivService;
        ...
        try {
            ...
            if (errores.isEmpty()) {
                if (!chips.isEmpty()) {
                    ...
                    Integer actualizados = chipVentanaSivService.saveVentanaSiv(chips);
