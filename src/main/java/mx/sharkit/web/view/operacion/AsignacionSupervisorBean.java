...
@Named
@ViewScoped
public class AsignacionSupervisorBean implements Serializable {

    private static final Logger LOG = Logger.getLogger(AsignacionSupervisorBean.class.getName());
    ...
    public void onCreateAsignacion() {
        try {
            if (StringUtils.equals("SSID", tipoAsignacion)) {
                ...
                if (chip != null) {
                    if (Objects.equals(chip.getEstatusId(), Estatus.ID_ESTATUS_ASIGNADO_DISTRIBUIDOR)) {
                ...
                Integer noActualizados = chipService.asignarChipsSubdistribuidorByRango(serie, serieFinal,
    ...
    public void obtenerChips() {
        try {
            ...
            dc.add(Restrictions.eq("estatusId", Estatus.ID_ESTATUS_ASIGNADO_SUPERVISOR));
