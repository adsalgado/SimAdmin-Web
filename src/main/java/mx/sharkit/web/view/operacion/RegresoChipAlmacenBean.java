...
@Named
@ViewScoped
public class RegresoChipAlmacenBean implements Serializable {
    
    private static final Logger LOG = Logger.getLogger(RegresoChipAlmacenBean.class.getName());
    ...
    public void onCreateAsignacion() {
        try {
            if (StringUtils.equals("SSID", tipoAsignacion)) {
                ...
                if (chip != null) {
                    if (Objects.equals(chip.getEstatusId(), Estatus.ID_ESTATUS_ASIGNADO_VENDEDOR)) {
                        ...
                        if (chipHistoricoService.saveChipAndHistory(chip, historicoEstatus)) {
                            obtenerChips();
                            PFMessages.showMessageInfo("El chip con numero: " + serie + " ha sido actualizado correctamente.");
                            cleanSearch();
                        } else {
                        ...
                        PFMessages.showMessageError("El chip con numero: " + serie + " se encuentra en estatus inválido para esta operación.");
                    }
                } else {
                    PFMessages.showMessageError("El chip con numero: " + serie + " no ha sido registrado");
                }
            } else {
                Integer noActualizados = chipService.regresarChipsAlmacenByRango(serie, serieFinal,
